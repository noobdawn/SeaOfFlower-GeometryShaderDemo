...
Shader "Unlit/Grass"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			void MakeList(int vertexCount, float3 root, float3 right, float3 look, inout g2f v[18])
			{
				float2 wind = float2(_CosTime.z + frac(root.x), _SinTime.z + frac(root.y));
				float3 up = float3(0, 1, 0);
				float currentV = 1.f;
				float offsetV = 1.f /((vertexCount / 2) - 1);
				//根据风向和风力，计算顶点弯曲到哪
				float2 windDir = normalize(wind);
				float windForce = length(wind) * UNITY_HALF_PI * _WindScale;
			...
			void geom(point v2g p[1], inout TriangleStream<g2f> triStream)
			{
				...
				if (lookDist > 100)
				{
					MakeList(4, p[0].pos, right, look, v);
				...
				{
					MakeList(8, p[0].pos, right, look, v);
					//将世界坐标系下的几个点转到屏幕空间，然后传入到流中
				...
				{
					MakeList(12, p[0].pos, right, look, v);
					//将世界坐标系下的几个点转到屏幕空间，然后传入到流中
